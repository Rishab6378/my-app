...
import './WomenWine.css';
import WomenCard from './WomenCard';

export class WomenWine extends Component {
    articles = [
        {
            "Price": "$82.66",
            ...
            <div className="ZXS11">
                {this.state.articles.map((element) => {
                    return <div key={element.url}>
                        <WomenCard title={element.title} description={element.description} imageUrl={element.urlToImage} priCe={element.Price} />
                    </div>
                })}
            </div>
        )
