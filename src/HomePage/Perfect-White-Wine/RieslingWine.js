...
import './RieslingWine.css';
import RieslingCard from './RieslingCard';

export class RieslingWine extends Component {
    articles = [
        {
            "Price": "$82.66",
            ...
            <div className="ZXS11">
                {this.state.articles.map((element) => {
                    return <div key={element.url}>
                        <RieslingCard title={element.title} description={element.description} imageUrl={element.urlToImage} priCe={element.Price} />
                    </div>
                })}
            </div>
        )
